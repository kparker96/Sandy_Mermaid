...
ggplot(sym_commun, aes(x = Sample.Name, y = value, fill = sym)) +
  ...
  labs(fill = "") +
  scale_fill_manual(values = c("#009E73", "#0072B2", "#CC79A7", "#F0E442")) +
  theme_classic() +
...
ggplot(sym_commun_depth, aes(x = newcol, y = value, fill = sym)) +
  ...
  theme(axis.title.x = element_text(color = "transparent"),
        ...
        axis.text.y = element_text(size = 30, color = "black", margin = margin(l = 30), angle = 90),
        panel.background = element_rect(color ="black"),
...
#Symbiont Dominance VS Depth, Mermaid Reef and Sandy Cay data grouped together WITHOUT REGRESSION LINES
```{r}
...
ggsave(filename = "figures/grouped_depth.png", width = 380, height = 400, units = "mm")
...
depth_dom_sym %>% group_by(site) %>% summarise(min = min(depth), max = max(depth))
depth_dom_sym <- depth_dom_sym %>%
...
ggplot(grouped_depth, aes(x = as.factor(depth_int), y = ncol, fill = dom_sym)) +
  geom_bar(stat = "identity", position = "fill") +
  geom_text(aes(x = as.factor(depth_int), y = 1.01, label = label), size = 8, inherit.aes = FALSE) +
  scale_fill_manual(values = c(  "#009E73", 	"#CC79A7", "#56B4E9", "#999999")) +
  geom_vline(xintercept = 1.5, lty = 2, size = 1.5) +
  xlab("Depth(m)") +
  ylab("Proportion  of Dominant Genera") +
  annotate("text", x = 1.62, y = 1.04, size = 9, label = "Mermaid Reef      Sandy Cay Reef →") +
  annotate("text", x = 2.9, y = 1.05, size = 20, color = "transparent", label = (expression(symbol('\256')))) +
  scale_y_continuous(expand = c(0,0)) +
  theme_classic() +
...
ggsave(filename = "figures/grouped_depth.png", width = 380, height = 380, units = "mm")
