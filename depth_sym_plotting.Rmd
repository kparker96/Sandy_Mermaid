...
#Principal COmponent Analysis of SC
```{r}
library(ggfortify)


pca_data <- sym_combos %>%
  select(A.Orb, B.Orb, C.Orb, D.Orb)

pca <- prcomp(pca_data, scale = FALSE, center = FALSE)

autoplot(pca, data = sym_combos, loadings = TRUE, loadings.label = TRUE)
```
...
net_data_SC <- sym_combos %>%
  mutate(total = A.Orb + B.Orb + C.Orb + D.Orb, 
         ...
         C.prop = C.Orb/total,
         D.prop = D.Orb/total) %>%
  select(Sample.Name, A.prop, B.prop, C.prop, D.prop)
...
plot(net_SC, edge.curved = 0.1, edge.width = 10 * (E(net_SC)$weight)^2, vertex.label = V(net_SC)$lab, 
     vertex.size = ifelse(grepl("SC", names(V(net_SC))), 10, 20),
     vertex.color = ifelse(grepl("SC", names(V(net_SC))), 
                           colorRampPalette(c("white", "black"))(100)[as.numeric(cut(V(net_SC)$depth_SC,breaks = 100))],
                           rainbow(4)))
```
...
plot(net_MR, edge.curved = 0.1, edge.width = 10 * (E(net_MR)$weight)^2, vertex.label = V(net_MR)$lab, 
     vertex.size = ifelse(grepl("MR", names(V(net_MR))), 10, 20),
     vertex.color = ifelse(grepl("MR", names(V(net_MR))), 
                           colorRampPalette(c("white", "black"))(100)[as.numeric(cut(V(net_MR)$depth,breaks = 100))],
                           rainbow(4)))
