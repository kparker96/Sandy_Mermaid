...
symbiont_comp_f <- symbiont_comp + 
  ...
  theme(legend.position = "bottom", 
        ...
        strip.text = element_text(size = 8),
        axis.text.x = element_text(size = 0, margin = margin(t = -20)),
...
ggsave(filename = "figures/host_symbiont_barplot.png", width = 174, height = 100, units = "mm")
```
...
ggplot(grouped_depth, aes(x = as.factor(depth_int), y = ncol, fill = dom_sym)) +
  geom_bar(stat = "identity", position = "fill") +
  geom_text(aes(x = as.factor(depth_int), y = 1.01, label = label), size = 1, inherit.aes = FALSE) +
  scale_fill_manual(values = c('#a6cee3','#1f78b4','#b2df8a','#33a02c')) + 
  geom_vline(xintercept = 1.5, lty = 2, lwd = 0.5) +
  xlab("Depth (m)") +
  ylab("Proportion  of colonies") +
  annotate("text", x = 1.62, y = 1.04, size = 2, label = "  Mermaid Reef     Sandy Cay Reef →") +
...
ggsave(filename = "figures/grouped_depth.png", width = 85, height = 85, units = "mm")
