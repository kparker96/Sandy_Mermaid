...
#removing "bad" data 
qpcr_bad <- qpcr_good %>%
  select(Sample.Name, Orb.CT.mean, A.CT.sd, B.CT.sd, C.CT.sd, D.CT.sd)
```
...
qpcr_dupes_f2 <- qpcr_dupes_f1 %>%
  mutate(sd_mean = mean(c(A.CT.sd, B.CT.sd, C.CT.sd, D.CT.sd, Orb.CT.sd), na.rm = TRUE))
```
...
```{r}
sym_combos <- qpcr_good %>%
  mutate(symbiont = case_when(A.Orb > 0 & B.Orb > 0 & C.Orb == 0 & D.Orb == 0 ~ "AB",
                             A.Orb > 0 & B.Orb == 0 & C.Orb > 0 ~ "AC",
                             A.Orb & D.Orb ~ "AD",
                             B.Orb & C.Orb ~ "BC",
                             B.Orb & D.Orb ~ "BD",
                             C.Orb & D.Orb ~ "CD",
                             A.Orb & B.Orb & C.Orb & D.Orb ~ "ABCD",
                             A.Orb & B.Orb & C.Orb & D.Orb == 0 ~ "ABC",
                             A.Orb & B.Orb & D.Orb & C.Orb == 0 ~ "ABD",
                             B.Orb & C.Orb & D.Orb ~ "BCD",
                             A.Orb & C.Orb & D.Orb ~ "ACD",
                             B.Orb & C.Orb & D.Orb ~ "BCD",
